...
dan_predTF <- function(scores1, scores2, lab, data_fit) {
  ...
  return(class_predTF(data_fit[,lab.tf1_x],data_fit[,lab.tf2_y],scores1,scores2,t=res_varRep$t,sigma=res_varRep$sigma,level=0.99,plot=T))
}
# Set some scores
# For ELK1 vs ETS1
...
result = dan_predTF(scores.x, scores.y, lab, data_fit)
print(result)
